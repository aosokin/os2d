...
This file is a changed version of
https://github.com/timy90022/One-Shot-Object-Detection/blob/master/lib/roi_data_layer/roibatchLoader.py
"""
...
from .minibatch import get_minibatch, imread


class roibatchLoader(data.Dataset):
  def __init__(self, roidb, ratio_list, ratio_index, query, batch_size, classes,
                     training=True, normalize=None):
    self._roidb = roidb
    self._query = query

    classes_to_detect = [cl for cl in classes if not cl == '__background__']

    self._num_classes = len(classes_to_detect)
    ...
    self._cat_ids = classes_to_detect
  ...
  def __getitem__(self, index):
    ...
    if self.training:
        ...
        if ratio < 1:
            ...
            pad_size = max(data_height, data_width)
            padding_data = torch.FloatTensor(pad_size, pad_size, 3).zero_()
            padding_data[:data_height, :data_width, :] = data[0]
            gt_boxes[:, :4].clamp_(0, pad_size)
            im_info[0, 0] = pad_size
            im_info[0, 1] = pad_size
  ...
  def load_query(self, choice, id=0):
    ...
    # rgb -> bgr
    # im = im[:,:,::-1]
    if random.randint(0,99)/100 > 0.5 and self.training and cfg.TRAIN.USE_FLIPPED:
      im = im[:, ::-1, :]
